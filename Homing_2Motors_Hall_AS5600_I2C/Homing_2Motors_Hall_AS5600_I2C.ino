...
const float HOMING_FAST_SPEED = 800.0;   // pasos/s (búsqueda rápida)
const float HOMING_FINE_SPEED = 200.0;   // pasos/s (búsqueda fina)
const float HOMING_ACCEL = 400.0;        // pasos/s^2
...
uint16_t readAS5600_raw(TwoWire &i2c) {
  ...
  if (i2c.endTransmission(false) != 0) return 0; // error
...
bool homingAccel(AccelStepper &motor, uint8_t hallPin, float fastSpeed, float fineSpeed, float accel, long &outCentroSteps) {
  ...
  while (digitalRead(hallPin) == LOW) { // si ya está LOW, salimos avanzando un poco
    motor.runSpeed();
    if (millis() - startT > TIMEOUT_MS) return false;
  }
  ...
  for (int i = 0; i < BACKOFF_STEPS; ++i) motor.runSpeed(); // pequeño backoff
  ...
  for (int i = 0; i < BACKOFF_STEPS; ++i) motor.runSpeed();
...
void loop() {
  ...
  if (ledFinalPhase && ledFinalCount < 4) {
    if (now - ledFinalLastToggle >= 1000) { // 1s ON/OFF
  ...
  if (motor1Centrado) {
    ...
    // normalizar:
    if (angRel1 > 180.0) angRel1 -= 360.0;
    if (angRel1 < -180.0) angRel1 += 360.0;
    // imprime cada 200ms
  ...
  if (motor2Centrado) {
    ...
    if (angRel2 > 180.0) angRel2 -= 360.0;
    if (angRel2 < -180.0) angRel2 += 360.0;
